...
export class StructureValidator {
  ...
  validateStructurePreservation(
    ...
    const newImports = modifiedStructure.imports.filter(imp => 
      !originalStructure.imports.some(orig => 
        orig.trim() === imp.trim() || 
   ...
   */
  validateStructurePreservationRelaxed(
    modifiedContent: string, 
    originalStructure: FileStructure
  ): ValidationResult {
  ...
  analyzeFileStructure(content: string): StructureAnalysis {
    ...
    const importPaths = structure.imports.map(imp => this.extractImportPath(imp)).filter(Boolean) as string[];
    
    // Determine export types
    const exportTypes = structure.exports.map(exp => {
      if (exp.includes('export default')) return 'default';
      if (exp.includes('export const')) return 'const';
      if (exp.includes('export function')) return 'function';
      if (exp.includes('export class')) return 'class';
      return 'other';
    });
    ...
    if (complexityScore > 20) complexity = 'high';
...
4. Preserve component names and function signatures
  ...
  private extractJSXElements(content: string): string[] {
    ...
    return [...new Set(jsxMatches.map(match => match.match(/<([A-Z][A-Za-z0-9]*)/)?.[1]).filter(Boolean))] as string[];
  }
  ...
  private aggressiveFileRepair(
    ...
    try {
      ...
      const componentRegex = new RegExp(
        `(function\\s+${componentName}|const\\s+${componentName}\\s*=|export\\s+default\\s+function\\s+${componentName})([\\s\\S]*?)(?=\\n\\s*export|\\n\\s*$|$)`,
        'i'
      );
